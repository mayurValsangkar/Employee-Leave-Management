...
public class AdminController {
	...
	@RequestMapping("/admdashboard/calender")
	public String showCalender(){return "admin/calender/calender";}
	...
	public String addEmployee(@ModelAttribute("employee") Employee emp, Model model){
		...
		if (emailExists) {
			...
			// Return the view name to display the form with the error message
			return "exceptions/emailAlreadyExists";
		}else {
	...
	public String employeeEdited(@ModelAttribute("employee") Employee emp){
		if(emp.getEid() != 0){
			...
			String message = "Dear "+emp.getFirstname()+""
					...
					+ "\nKindly use the below password from your next session on Employee Leave Management System."
					+ "\n\n"
					+ "New Password: "+temp+"";
